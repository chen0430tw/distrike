...
- `F(t)` = 时刻 t 的剩余空间
- `K` = 斩杀线阈值
- `C(t)` = 缓存/日志等的自然增长速率（GB/天）
- `S(F)` = 系统自耗函数，当剩余空间不足时系统自身额外消耗的空间
...
其中 S(F) = α · (K - F(t))²/K,  α > 0
```

`S(F)` 是正反馈项——剩余空间越少，系统自耗越大：
- pagefile 扩容速率与可用空间成反比
- Windows Update 失败回滚产生额外占用
- NTFS 碎片化导致元数据膨胀

**斩杀线条件**：当 `dF/dt` 的负向加速度使系统无法通过正常清理恢复时，定义该临界点为斩杀线。

设用户清理恢复速率为 `R`（一次 `distrike clean` 能释放的空间速率），则存活条件为：

```
R > C(t) + S(F)
```

当 `F → 0` 时，`S(F) → αK`，系统自耗趋向最大值。若 `R < C(t) + αK`，则**不可恢复**。

斩杀线 `K` 的最优值为使 `S(K) = 0` 的点，即系统自耗刚好为零的临界剩余空间：

```
K = B_page + B_update + B_vhdx + B_app
  = 5 + 8 + 3 + 3
  = 19 ≈ 20 GB
```
